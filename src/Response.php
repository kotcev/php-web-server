...
class Response
{
    ...
    public function __construct(string $body = "", int $statusCode = self::HTTP_OK, array $headers = [])
    {
    ...
    {
        if (isset($value)) {
            return array_push($this->headers, [ $key => $value ]);
        }
    ...
    {
        $resp = "HTTP/1.1 " . self::$messages[$this->statusCode] . "\r\n";
        ...
        foreach ($headers as $hKey => $hVal) {
            ...
            if ( ! empty($hVal)) {
                ...
                $resp .= ucfirst($hKey) . ": " . $hVal;

                if ($hKey != $headersLength) {
                    $resp .= self::HEADER_SEPARATOR;
                }
            }
        }

        $this->responseString = $resp . self::HEADERS_BODY_SEPARATOR . $this->body;
    }
    ...
    {
        $this->defaultHeaders['date'] = gmdate('D, d M Y H:i:s T');
        $this->defaultHeaders['X-Powered-By'] = "Kotcev Web Server";
    }
}